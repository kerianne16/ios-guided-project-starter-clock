          ...
          let clockCenter = CGPoint(x: rect.size.width / 2.0,
           ...
           for i in 1...12 {
              ...
              let labelX = clockCenter.x + (numeralDistanceFromCenter - digitFont.lineHeight / 2.0)
               ...
               let labelY = clockCenter.y - 1 * (numeralDistanceFromCenter - digitFont.lineHeight / 2.0)                    * CGFloat(sin((Double.pi / 180) * Double(i + offset) * 30))
              hourString.draw(in: CGRect(x: labelX - digitFont.lineHeight / 2.0,
                                         y: labelY - digitFont.lineHeight / 2.0,
                                         width: digitFont.lineHeight,
                                         height: digitFont.lineHeight),                               withAttributes: [NSAttributedString.Key.foregroundColor: digitColor,
            ...
            // this creates a bounding square centered directly over the clock center
            // to draw a circle into
